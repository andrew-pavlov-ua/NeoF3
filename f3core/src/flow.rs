...
impl Flow {
    pub fn new(total_size: u64, max_process_rate: i64, progress: bool) -> Flow {
        Flow {
            ...
            max_process_rate: if max_process_rate <= 0 {f64::MAX} else {(max_process_rate * 1024) as f64 }, // Convert Kb/s to bytes/s
            measured_blocks: 0,
            measured_time_ms: 0,
            processed_blocks: 0,
            acc_delay_us: 0,
    ...
    pub fn report_progress(&mut self) {
        ...
        let (inst_speed, unit) = adjust_unit(inst_speed);
        ...
        let mut progress_str =
            format!("{:.2}% -- {:.2} {}/s", percent, inst_speed, unit);
    ...
    pub fn get_remaining_chunk_size(&self) -> usize {
            if self.processed_blocks as i64 >= self.blocks_per_delay {
            return 0;
        }
        let left_blocks = (self.blocks_per_delay - self.processed_blocks as i64) as usize;
        left_blocks.saturating_mul(self.block_size as usize)
    }
    ...
    pub fn measure(&mut self, file: &File, processed: i64) -> Result<()> {
        ...
        let mut delay: u64 = (end_time - self.measure_start_time + Duration::from_micros(self.acc_delay_us) ).as_millis() as u64;
        ...
        if delay < self.delay_ms && inst_speed > self.max_process_rate {
            let mut wait_ms = ((bytes_k - delay as f64 * self.max_process_rate) / self.max_process_rate).round();
    ...
    fn adjust_state(&mut self, inst_speed: f64, delay: u64){
        ...
        let below = self.is_rate_below(delay, inst_speed);
         ...
         match self.state {
            Inc => {
                if above {
                   self.move_to_search(self.blocks_per_delay - self.step / 2, 
            ...
            Dec => {
                if above {
                    ...
                    self.move_to_search(self.blocks_per_delay, 
